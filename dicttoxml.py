...
def convert_list(items, ids, parent, attr_type, item_func, cdata):
    ...
    for i, item in enumerate(items):
        ...
        if isinstance(item, numbers.Number) or type(item) in (str, unicode):
            ...
            addline(
                convert_kv(index_item_name, item.isoformat(), attr_type, attr, cdata)
            )

        elif type(item) == bool:
            addline(convert_bool(index_item_name, item, attr_type, attr, cdata))
            ...
            addline(convert_none(index_item_name, None, attr_type, attr, cdata))
